...
//새로 create과팅
router.post("/creategwa", (req, res) => {
  const _id = req.body.nickname; //닉네임
  const real_id = req.body.userid; //아이디
  const gender = req.body.gender;
  const num = req.body.num; //인원수
  const content = req.body.content; //소개글
  const openkakao = req.body.openkakao;
  const box = {};
  box.boolean = false;
  connection.query(
    "SELECT * FROM party_table where party_realid = ?",
    [real_id],
    function (err, rows, fields) {
      if (rows[0] === undefined) {
        console.log(rows[0]);
        connection.query(
          "INSERT INTO party_table (party_realid, party_userid, party_gender, party_number, party_body, party_address) values (?, ?, ?, ?, ?, ?)",
          [real_id, _id, gender, num, content, openkakao],
          function (err, rows, fields) {
            if (err) {
              // console.log(err);
            } else {
...
//과팅읽어오기 new
router.post("/readgwa", (req, res) => {
  connection.query("SELECT * from party_table", function (err, rows, fields) {
    if (err) {
      console.log(err);
      console.log("과팅목록에러");
    } else if (rows[0] != undefined) {
